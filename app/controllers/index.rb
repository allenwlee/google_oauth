get '/' do #here we send our key, secret to Google and the redirect, and the scope of what we want to access.  the user hasn't logged in yet.
...
get '/oauth2callback' do #we just sent our key, secret to Google.  Google is now sending back the particular code for the user who just entered their username/password.  it sends back validation and what we the app are authorized to use.
  p params
  @client = OAuth2::Client.new(
    ENV['GOOGLE_CLIENT_ID'],
    ENV['GOOGLE_CLIENT_SECRET'],
  ...
  token = @client.auth_code.get_token(params[:code], :redirect_uri => 'http://localhost:9393/oauth2callback')
